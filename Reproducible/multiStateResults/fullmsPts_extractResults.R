...
source(file = "functions/moreBurnin.R", local = T)
burnNeeded <- TRUE
...
for(s in S){
  ...
  for(pr in prior){
    ...
    for(N in N_test){
      ...
      for(p in p_test){
        cat("\t\t\t", p, "\n")
        for(a in alpha_test){
          directory <- paste0("./results/", model, "_S", s, "_", pr)
          ...
          for(iter in 1:10){
            ...
            if(burnNeeded)
              samples <- moreBurnin(model, samples)
  ...
  write.csv(testSummaries,
            file=paste0("./results/summaries/", model, "_S", s, "_Results.csv"), 
            row.names = F)
...
resS5 <- read.csv(paste0("./results/summaries/", model, "_S5_Results.csv"))
resS7 <- read.csv(paste0("./results/summaries/", model, "_S7_Results.csv"))
resS9 <- read.csv(paste0("./results/summaries/", model, "_S9_Results.csv"))

allres <-  resS5 %>% 
  rbind(resS7[,!grepl("p[0-9][6-9]", colnames(resS7))]) %>% 
  rbind(resS9[,!grepl("p[0-9][6-9]", colnames(resS9))])

write.csv(allres, file=paste0("./results/summaries/", model, "_AllResults.csv"), row.names = F)
